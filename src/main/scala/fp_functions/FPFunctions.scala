...
object FPFunctions {
    ...
    def foldR[A, B](xs: List[A], f: (A, B) => B, init: B): B = {
        val reversedList = reverse(xs)
        foldL(reversedList, (b:B,a:A)=> f(a,b), init)
    }
